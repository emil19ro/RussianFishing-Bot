...
async def spining():
    ...
    while not keyboard.is_pressed('P'):
        ...
        if not ready and current_state != STATES['RETRIEVING']:
            pyautogui.press('space')
            pyautogui.keyDown('[')
            pyautogui.keyUp('shift')
            current_state = STATES['RETRIEVING']

        if take_rod_up and current_state == STATES['RETRIEVING'] and not tackle_ready_check:
            while keyboard.is_pressed('['):
                pyautogui.keyUp('[')
            pyautogui.keyDown(']')
            pyautogui.keyDown('shift')
            current_state = STATES['ROD_UP']
